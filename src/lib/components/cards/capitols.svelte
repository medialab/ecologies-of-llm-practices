...
export let transitionDelay
        ...
        >
            ...
            <div class="w-[50px] h-[50px] max-md:w-[40px] max-md:h-[40px] absolute top-0 max-md:top-[10px] right-[10px] z-20 flex items-center justify-center p-2" style="right: {$isMobileDevice ? '10px' : '0px'};">
                <button
            ...
            <div class="w-full relative flex flex-col col-start-1 col-end-8 max-md:col-start-1 max-md:col-end-2 max-md:row-start-1 z-5 pointer-events-none row-start-1 mb-[3vw] max-md:p-[15px] max-md:mb-[1vw]" style="background-color: {card.bgColor}; border: 5px solid {card.bgColor};"> 
                <h1 class=" whitespace-normal select-none pointer-events-auto overflow-visible inline-block mb-[-0.2em] mt-[-0.18em] translate-x-[-0.08em] min-[1920px]:text-[6.5vw] min-[1920px]:mt-[-0.14em]" style="z-index: 7;">
                    {@html card.Title}
                </h1>
            </div>

            <div class="w-full h-[calc(100%-2px)] md:col-start-2 md:col-end-8 max-md:col-start-1 max-md:col-end-2 row-start-2 max-md:row-start-2 mt-[0.1%] pt-[1%] pr-[var(--spacing-XS)] overflow-y-auto overflow-x-hidden z-[1]"
                data-lenis-prevent
                data-section={card.Title}>
                
                <div class="flex flex-col gap-4 w-full md:pr-6 px-4" data-section={card.Title}>

                    <div class="flex flex-col gap-2">
                        {#if card.Question}
                            <p class="md:text-2xl text-xl md:w-2/3 w-full leading-[120%]">
                                {@html card.Question}
                            </p>
                        {/if}

                        {#if card.Description}
                            <p class="md:text-lg w-full">{@html card.Description}</p>
                        {/if}
                    </div>
                        ...
                        onfocus={() => {
                            ...
                            {#if section.title}
                                <div class="w-full h-fit flex flex-col gap-2">
                                    <div class="w-full flex items-center gap-[var(--spacing-M)] max-md:gap-[var(--spacing-XS)]">
                                        <h2 class="md:text-5xl text-3xl max-md:w-fit md:text-nowrap items-center justify-center flex gap-2">{@html section.title}
                                            <button 
                                                type="button"
                                                onclick={(event) => {
                                                    event.preventDefault();
                                    ...
                                    {#if section.subtitle }
                                        <p class="md:text-xl md:w-2/3">{@html section.subtitle}</p>
                                    {/if}
                                </div>
                            {/if}
                            ...
                            {/if}
                             ...
                             {#if section.text}
                                <div class="w-full h-full">
                                    <p class="md:text-xl w-full hyphens-auto break-words overflow-wrap-anywhere">{@html section.text}</p>
                                </div>
                            {/if}
                        </div>
